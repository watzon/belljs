...
export class FloodWaitError extends FloodError {
  ...
  constructor(args: any) {
    ...
    this.message = `A wait of ${seconds} seconds is required` +
      RPCError._fmtRequest(args.request);
    this.seconds = seconds;
  }
}
...
export class NetworkMigrateError extends InvalidDCError {
  ...
  constructor(args: any) {
    ...
    this.message = `The source IP address is associated with DC ${newDc}` +
      RPCError._fmtRequest(args.request);
    this.newDc = newDc;
  }
}
...
export class EmailUnconfirmedError extends BadRequestError {
  ...
  constructor(args: any) {
    ...
    super(
      `Email unconfirmed, the length of the code must be ${codeLength}${
        RPCError._fmtRequest(
          args.request,
        )
      }`,
      args.request,
...
export class MsgWaitError extends FloodError {
  constructor(args: any) {
    ...
    this.message = `Message failed to be sent.${
      RPCError._fmtRequest(
        args.request,
      )
    }`;
